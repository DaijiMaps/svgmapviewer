...
def gen_labels(names_txt: str) -> None:
    ...
    with open(f"/tmp/names-patterns.txt", "w", encoding="utf-8") as fh:
        names = {}
...
def get_bboxes() -> None:
    p = subprocess.Popen(['inkscape', '-S', '/tmp/gen-labels-names.svg'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def write_scaled_names_txt() -> None:
    ...
    if not output_json:
        for l in ls:
            # XXX area
            xs = [l["name"], l["pat"], str(l["s"]), str(l["dy"])]
            ys.append('@@@'.join(xs))
        print('\n'.join(ys))
    else:
        sys.stdout.reconfigure(encoding='utf-8')
        json.dump(ls, sys.stdout, ensure_ascii=False, indent=2)
